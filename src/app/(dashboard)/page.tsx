...
export default function DashboardPage() {
  const { selectedProject, selectedReport, loading, error } =
    useReportContext();
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    exporter.addKeyValue(
      "Contract Type:",
      selectedProject?.contractType || "N/A"
    );
    exporter.addKeyValue(
      "Contract Price:",
      `$${((selectedProject?.contractPrice || 0) / 1e6).toFixed(2)}M`
    );
    exporter.addKeyValue(
      "Duration:",
      `${selectedProject?.startDate || ""} - ${
        selectedProject?.finishDate || ""
      }`
    );
    exporter.addSpacing();
    ...
    const progressStatus =
      (progress.variance || 0) >= 0
        ? "good"
        : (progress.variance || 0) >= -5
        ? "warning"
        : "bad";
    const spiStatus =
      (evm.spiValue || 1) >= 1
    ...
    exporter.addStatsRow([
      {
        label: "Progress",
        value: `${(progress.actual || 0).toFixed(1)}%`,
        status: progressStatus,
      },
      {
        label: "SPI",
        value: (evm.spiValue || 1).toFixed(3),
        status: spiStatus,
      },
      {
        label: "CPI",
        value: (evm.cpiValue || 1).toFixed(3),
        status: cpiStatus,
      },
    ]);
    exporter.addStatsRow([
      {
        label: "EAC",
        value: `$${(
          ((evm as { eac?: number }).eac || evm.bac || 0) / 1e6
        ).toFixed(2)}M`,
        status: "neutral",
      },
      {
        label: "Safe Hours",
        value: `${((selectedReport?.hse?.safeHours || 0) / 1000).toFixed(0)}K`,
        status: "neutral",
      },
      {
        label: "Cash Flow",
        value:
          selectedReport?.cashFlow?.overallStatus === "green"
            ? "Healthy"
            : selectedReport?.cashFlow?.overallStatus === "yellow"
            ? "Monitor"
            : "At Risk",
        status:
          selectedReport?.cashFlow?.overallStatus === "green"
    ...
    if (selectedProject) {
      ...
      const daysRemaining = selectedProject.finishDate
        ? Math.ceil(
            (new Date(selectedProject.finishDate).getTime() -
              new Date().getTime()) /
              (1000 * 60 * 60 * 24)
          )
        : 0;
      const adjustedDays =
        evm.spiValue > 0
          ? Math.ceil(daysRemaining / evm.spiValue)
          : daysRemaining;
      const delayDays = Math.max(0, adjustedDays - daysRemaining);
      const ldRate =
        (selectedProject as unknown as { ldDelayRate?: number }).ldDelayRate ||
        0.001;
      const ldAmount =
        delayDays * ldRate * (selectedProject.contractPrice || 0);

      exporter.addKeyValue(
        "Planned Finish:",
        selectedProject.finishDate || "N/A"
      );
      exporter.addKeyValue(
        "Delay Days:",
        delayDays <= 0 ? "On Schedule" : `${delayDays} days`
      );
      exporter.addKeyValue(
        "LD Delay Estimation:",
        `$${(ldAmount / 1e6).toFixed(2)}M`
      );
      exporter.addSpacing();
    }
    ...
    exporter.addStatsRow([
      {
        label: "BCWS",
        value: `$${((evm.bcws || 0) / 1e6).toFixed(2)}M`,
        status: "neutral",
      },
      {
        label: "BCWP",
        value: `$${((evm.bcwp || 0) / 1e6).toFixed(2)}M`,
        status: "neutral",
      },
      {
        label: "ACWP",
        value: `$${((evm.acwp || 0) / 1e6).toFixed(2)}M`,
        status: "neutral",
      },
      {
        label: "BAC",
        value: `$${((evm.bac || 0) / 1e6).toFixed(2)}M`,
        status: "neutral",
      },
    ]);
    exporter.addSpacing();
    ...
    if (selectedReport?.cashFlow) {
      ...
      exporter.addStatsRow([
        {
          label: "Revenue",
          value: `$${((cf.revenue || evm.bcwp || 0) / 1e6).toFixed(2)}M`,
          status: "good",
        },
        {
          label: "Cash Out",
          value: `$${((cf.cashOut || 0) / 1e6).toFixed(2)}M`,
          status: "bad",
        },
        {
          label: "Billing",
          value: `$${((cf.billing || 0) / 1e6).toFixed(2)}M`,
          status: "neutral",
        },
        {
          label: "Cash In",
          value: `$${((cf.cashIn || 0) / 1e6).toFixed(2)}M`,
          status: "neutral",
        },
      ]);
      // Cash Flow KPIs
      ...
      const billingCoverage =
        (cf.revenue || evm.bcwp || 1) > 0
          ? (cf.billing || 0) / (cf.revenue || evm.bcwp || 1)
          : 0;
      const cashCollection =
        (cf.billing || 1) > 0 ? (cf.cashIn || 0) / (cf.billing || 1) : 0;
      const cashAdequacy =
        (cf.cashOut || 1) > 0 ? (cf.cashIn || 0) / (cf.cashOut || 1) : 0;
      exporter.addStatsRow([
        {
          label: "Balance",
          value: `$${(balance / 1e6).toFixed(2)}M`,
          status: balance >= 0 ? "good" : "bad",
        },
        {
          label: "Billing Coverage",
          value: `${(billingCoverage * 100).toFixed(1)}%`,
          status: billingCoverage >= 0.9 ? "good" : "warning",
        },
        {
          label: "Cash Collection",
          value: `${(cashCollection * 100).toFixed(1)}%`,
          status: cashCollection >= 0.9 ? "good" : "warning",
        },
        {
          label: "Cash Adequacy",
          value: `${cashAdequacy.toFixed(2)}x`,
          status: cashAdequacy >= 1 ? "good" : "warning",
        },
      ]);
      exporter.addSpacing();
    }
    ...
    if (tkdn.plan > 0 || tkdn.actual > 0) {
      ...
      exporter.addStatsRow([
        ...
        {
          label: "Status",
          value: (tkdn.actual || 0) >= (tkdn.plan || 0) ? "PASS" : "AT RISK",
          status: tkdnStatus,
        },
      ]);
      exporter.addSpacing();
    ...
    if (selectedReport?.epcc) {
      ...
      const epcc = selectedReport.epcc as unknown as Record<
        string,
        { plan?: number; actual?: number; weight?: number }
      >;
      const epccData = [
        "engineering",
        "procurement",
    ...
    exporter.addStatsRow([
      {
        label: "Fatality",
        value: String(lagging.fatality || 0),
        status: (lagging.fatality || 0) === 0 ? "good" : "bad",
      },
      {
        label: "LTI",
        value: String(lagging.lti || 0),
        status: (lagging.lti || 0) === 0 ? "good" : "bad",
      },
      {
        label: "Medical",
        value: String(lagging.medicalTreatment || 0),
        status: "neutral",
      },
      {
        label: "First Aid",
        value: String(lagging.firstAid || 0),
        status: "neutral",
      },
    ]);
    exporter.addText(
      `Safe Hours: ${(hse.safeHours || 0).toLocaleString()}  |  TRIR: ${(
        hse.trir || 0
      ).toFixed(2)}  |  Manpower: ${
        (hse.manpower as { total?: number })?.total || 0
      }`,
      "small"
    );
    exporter.addSpacing();
    ...
    // Quality
    const quality = (selectedReport?.quality || {}) as Record<
      string,
    ...
    exporter.addStatsRow([
      {
        label: "Certificates",
        value: String(quality?.certificate?.completed || 0),
        status: "good",
      },
      {
        label: "Under Application",
        value: String(quality?.certificate?.underApplication || 0),
        status: "warning",
      },
      {
        label: "Not Yet Applied",
        value: String(quality?.certificate?.notYetApplied || 0),
        status: "neutral",
      },
    ]);
    ...
    // Milestones - Focus on incomplete, grouped by status
    const msSchedule = (selectedReport?.milestonesSchedule || []) as Array<{
      no?: number;
      description?: string;
      name?: string;
      planDate?: string;
    ...
    const msPayment = (selectedReport?.milestonesPayment || []) as Array<{
      no?: number;
      description?: string;
      name?: string;
      planDate?: string;
    ...
    const statusColors = {
      delayed: { r: 239, g: 68, b: 68 }, // Red
      atRisk: { r: 245, g: 158, b: 11 }, // Amber
      onTrack: { r: 59, g: 130, b: 246 }, // Blue
      completed: { r: 34, g: 197, b: 94 }, // Green
    };

    // Helper function to process milestones
    const processMilestones = (milestones: typeof msSchedule) => {
      // Normalize status for comparison (case-insensitive)
      const normalizeStatus = (status?: string) =>
        (status || "").toLowerCase().trim();

      // Filter out completed (match various formats: Completed, Complete, etc.)
      const incomplete = milestones.filter((m) => {
        const status = normalizeStatus(m.status);
        return !status.startsWith("complete");
      });

      // Group by status priority
      const delayed = incomplete.filter((m) => {
        const status = normalizeStatus(m.status);
        return (
          status === "delayed" || status === "delay" || status === "critical"
        );
      });
      const atRisk = incomplete.filter((m) => {
        const status = normalizeStatus(m.status);
        return (
          status === "at risk" || status === "pending" || status === "atrisk"
        );
      });
      const onTrack = incomplete.filter((m) => {
        const status = normalizeStatus(m.status);
        return (
          status === "on track" ||
          status === "ontrack" ||
      ...
      // Sort each group by plan date (newest first)
      const sortByDate = (
        a: (typeof milestones)[0],
        b: (typeof milestones)[0]
      ) => {
      ...
      delayed.sort(sortByDate);
      atRisk.sort(sortByDate);
      onTrack.sort(sortByDate);
      completed.sort(sortByDate);

      return { delayed, atRisk, onTrack, completed };
    };

    // Helper to build table rows for a status group
    const buildTableRows = (
      milestones: typeof msSchedule,
      statusLabel: string,
      color: { r: number; g: number; b: number }, // Now used as background color
      limit = 5
    ) => {
      const rows: Array<
        Array<{
      ...
      displayed.forEach((m, idx) => {
        ...
        rows.push([
          { text: String(m.no || idx + 1), align: "center" },
          { text: desc, align: "left" },
          { text: m.planDate || "-", align: "center" },
          { text: m.actualForecastDate || "-", align: "center" },
          {
            text: statusLabel,
            type: "badge",
      ...
      return { rows, remaining: milestones.length - limit };
    };

    // Table headers
    const tableHeaders = [
      { label: "#", width: 10, align: "center" as const },
      { label: "Description", width: 85, align: "left" as const },
      { label: "Plan Date", width: 28, align: "center" as const },
      { label: "Actual/FC", width: 28, align: "center" as const },
      { label: "Status", width: 35, align: "center" as const },
    ];

    // Process and render Schedule Milestones
    const scheduleData = processMilestones(msSchedule);
    const hasScheduleData =
      scheduleData.delayed.length > 0 ||
      scheduleData.atRisk.length > 0 ||
      scheduleData.onTrack.length > 0 ||
      scheduleData.completed.length > 0;

    // Process and render Payment Milestones
    const paymentData = processMilestones(msPayment);
    const hasPaymentData =
      paymentData.delayed.length > 0 ||
      paymentData.atRisk.length > 0 ||
      paymentData.onTrack.length > 0 ||
      paymentData.completed.length > 0;

    if (hasScheduleData || hasPaymentData) {
      exporter.addSpacing();
      ...
      if (hasScheduleData) {
        ...
        if (scheduleData.delayed.length > 0) {
          const { rows, remaining } = buildTableRows(
            scheduleData.delayed,
            "Delayed",
            statusColors.delayed
          );
          scheduleRows.push(...rows);
          if (remaining > 0) scheduleOverflow += remaining;
        }
        if (scheduleData.atRisk.length > 0) {
          const { rows, remaining } = buildTableRows(
            scheduleData.atRisk,
            "At Risk",
            statusColors.atRisk
        ...
        if (scheduleData.onTrack.length > 0) {
          const { rows, remaining } = buildTableRows(
            scheduleData.onTrack,
            "On Track",
        ...
        if (scheduleData.completed.length > 0) {
          const { rows, remaining } = buildTableRows(
        ...
        if (scheduleRows.length > 0) {
          exporter.addTable(tableHeaders, scheduleRows);
        }
        if (scheduleOverflow > 0) {
          exporter.addText(
            `... and ${scheduleOverflow} more schedule milestones`,
            "small",
            { r: 100, g: 116, b: 139 }, // Gray
      ...
      if (hasPaymentData) {
        ...
        > = [];
        let paymentOverflow = 0;

        if (paymentData.delayed.length > 0) {
          const { rows, remaining } = buildTableRows(
            paymentData.delayed,
            "Delayed",
            statusColors.delayed
          ...
          if (remaining > 0) paymentOverflow += remaining;
        }
        if (paymentData.atRisk.length > 0) {
          const { rows, remaining } = buildTableRows(
            paymentData.atRisk,
            "At Risk",
            statusColors.atRisk
        ...
        if (paymentData.onTrack.length > 0) {
          const { rows, remaining } = buildTableRows(
            paymentData.onTrack,
            "On Track",
            statusColors.onTrack
        ...
        if (paymentData.completed.length > 0) {
          const { rows, remaining } = buildTableRows(
        ...
        if (paymentRows.length > 0) {
          exporter.addTable(tableHeaders, paymentRows);
        }
        if (paymentOverflow > 0) {
          exporter.addText(
            `... and ${paymentOverflow} more payment milestones`,
            "small",
            { r: 100, g: 116, b: 139 }, // Gray
            { align: "center", italic: true }
          );
    ...
    // Activities
    const thisWeek = (selectedReport?.thisWeekActivities || {}) as Record<
      string,
      string[]
    >;
    const nextWeek = (selectedReport?.nextWeekPlan || {}) as Record<
      string,
      string[]
    >;
    ...
    // Save
    const filename = `EPC_Dashboard_Week${selectedReport?.weekNo || ""}_${
  ...
  const overallProgress = selectedReport?.overallProgress || {
    plan: 0,
    actual: 0,
    variance: 0,
  };
  ...
  const spiColor =
    evm.spiValue >= 1 ? "#22c55e" : evm.spiValue >= 0.9 ? "#f59e0b" : "#ef4444";
  const cpiColor =
    evm.cpiValue >= 1 ? "#22c55e" : evm.cpiValue >= 0.9 ? "#f59e0b" : "#ef4444";
    ...
    <div className="space-y-5">
        ...
        <div>
          ...
          {selectedReport && (
            <p className="text-sm text-slate-500">
              Week {selectedReport.weekNo} |{" "}
              {selectedReport.periodStart || "N/A"} -{" "}
              {selectedReport.periodEnd || "N/A"}
            </p>
          )}
        </div>
        ...
        (() => {
          ...
          return (
            <div className="rounded-2xl bg-gradient-to-br from-teal-50 to-cyan-50 border-2 border-teal-200 p-4">
              <h3 className="text-sm font-bold text-teal-700 mb-3">
                📋 Project Information
              </h3>
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 mb-2">
                ...
                <div className="rounded-lg bg-white p-3">
                  ...
                  <p className="text-xs font-semibold">
                    {(selectedProject as unknown as { termOfPayment?: string })
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-5 mb-2">
                ...
                <div className="rounded-lg bg-white p-3">
                  ...
                  <p className="text-sm font-bold text-blue-600">
                    {(
                      selectedProject as unknown as { guaranteedPower?: number }
                    ).guaranteedPower || 0}{" "}
                    MW
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div className="rounded-lg bg-white p-3">
                  ...
                  <p className="text-xs font-semibold">
                    $
                    {(
                      (selectedProject as unknown as { ldDelay?: number })
                        .ldDelay || 0
                    ).toLocaleString()}
                    /day
                  </p>
                ...
                <div className="rounded-lg bg-white p-3">
                  ...
                  <p className="text-xs font-semibold">
                    $
                    {(
                      (selectedProject as unknown as { ldPerformance?: number })
                        .ldPerformance || 0
                    ).toLocaleString()}
                    /kW
                  </p>
                ...
                <div className="rounded-lg bg-white p-3">
                  ...
                  <p className="text-xs font-semibold">
                    {(selectedProject as unknown as { scopeByOwner?: string })
                      .scopeByOwner || "-"}
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
        ...
        <div className="rounded-2xl bg-white p-4 shadow-sm">
          ...
          <div className="mt-2 flex items-end gap-2">
            <span className="text-3xl font-extrabold text-teal-600">
              {(overallProgress.actual || 0).toFixed(1)}%
            </span>
            <span
              className={`text-sm font-semibold ${
                (overallProgress.variance || 0) >= 0
                  ? "text-green-500"
                  : "text-red-500"
              }`}
            >
              {(overallProgress.variance || 0) >= 0 ? "+" : ""}
              {(overallProgress.variance || 0).toFixed(1)}%
            </span>
          </div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200">
            <div
              ...
              style={{
                width: `${Math.min(overallProgress.actual || 0, 100)}%`,
              }}
            />
          </div>
          <p className="mt-1 text-[10px] text-slate-400">
            Plan: {(overallProgress.plan || 0).toFixed(1)}%
          </p>
        </div>
        ...
        {(() => {
          ...
          const calculatedEac =
            evm.cpiValue > 0 ? evm.bac / evm.cpiValue : evm.bac;
          const eacValue =
            storedEac && storedEac > 0 ? storedEac : calculatedEac;
          const vacValue = evm.bac - eacValue;
          return (
            <div
              className={`rounded-2xl p-4 shadow-sm ${
                vacValue >= 0 ? "bg-green-50" : "bg-red-50"
              }`}
            >
              <p className="text-xs font-semibold text-slate-500">
                📉 EAC (Typical)
              ...
              <p
                className={`mt-1 text-[10px] ${
                  vacValue >= 0 ? "text-green-600" : "text-red-600"
                }`}
              >
        ...
        <div className="rounded-2xl bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-[10px] text-slate-400">
            LTI: {(hse.lagging as Record<string, number>)?.lti || 0} | TRIR:{" "}
            {(hse.trir || 0).toFixed(2)}
          </p>
        </div>

        {/* Cash Flow Status - Use same real-time calculation as Dashboard */}
        {(() => {
          const cf = (selectedReport?.cashFlow || {}) as {
            cashIn?: number;
            cashOut?: number;
            billing?: number;
            revenue?: number;
          ...
          const billingOk =
            billingCoverage >= 0.95 ? 1 : billingCoverage >= 0.85 ? 0.5 : 0;
          const collectionRatio = billing > 0 ? cashIn / billing : 0;
          const collectionOk =
            collectionRatio >= 0.9 ? 1 : collectionRatio >= 0.8 ? 0.5 : 0;
          const adequacyRatio = cashOut > 0 ? cashIn / cashOut : 0;
          const adequacyOk =
            adequacyRatio >= 1.0 ? 1 : adequacyRatio >= 0.9 ? 0.5 : 0;

          const overallScore =
            (balanceOk + billingOk + collectionOk + adequacyOk) / 4;
          const overallStatus =
            overallScore >= 0.75
              ? "green"
              : overallScore >= 0.5
      ...
      {selectedProject &&
        (() => {
          ...
          const guaranteedPower =
            (selectedProject as unknown as { guaranteedPower?: number })
              .guaranteedPower || 0;
          const powerShortfall = Math.max(
            0,
            guaranteedPower - actualForecastPower
          );

          const ldDelayRate =
            (selectedProject as unknown as { ldDelay?: number }).ldDelay || 0;
          const ldPerformanceRate =
            (selectedProject as unknown as { ldPerformance?: number })
              .ldPerformance || 0;
            ...
            >
              <h3
                className={`mb-4 text-sm font-bold ${
                  totalLD > 0 ? "text-red-700" : "text-green-700"
                }`}
              >
                ⏱️ Schedule & LD Estimation
              </h3>
              ...
              <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4 mb-3">
                ...
                <div className="rounded-xl bg-white p-3 text-center">
                  ...
                  <p
                    className={`text-sm font-bold ${
                      delayDays > 0 ? "text-red-600" : "text-green-600"
                    }`}
                  >
                    {estimatedCompletion}
                  </p>
                ...
                <div
                  className={`rounded-xl p-3 text-center ${
                    delayDays <= 0 ? "bg-green-100" : "bg-red-100"
                  }`}
                >
                  <p className="text-[10px] text-slate-500">Delay Days</p>
                  <p
                    className={`text-xl font-extrabold ${
                      delayDays <= 0 ? "text-green-600" : "text-red-600"
                    }`}
                  >
                    {delayDays} days
                  </p>
                ...
                <div className="rounded-xl bg-white p-3 text-center">
                  ...
                  <p
                    className={`text-sm font-bold ${
                      actualForecastPower < guaranteedPower
                        ? "text-red-600"
                        : "text-green-600"
                    }`}
                  >
                    {actualForecastPower} MW
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div
                  className={`rounded-xl p-3 text-center ${
                    ldDelay > 0 ? "bg-red-100" : "bg-green-100"
                  }`}
                >
                  <p className="text-[10px] text-slate-500">
                    LD Delay Estimation
                  ...
                  <p
                    className={`text-lg font-extrabold ${
                      ldDelay > 0 ? "text-red-600" : "text-green-600"
                    }`}
                  >
                    ${ldDelay.toLocaleString()}
                  </p>
                  <p className="text-[9px] text-slate-400">
                    {delayDays} days × ${ldDelayRate.toLocaleString()}/day
                  </p>
                </div>
                <div
                  className={`rounded-xl p-3 text-center ${
                    ldPerformance > 0 ? "bg-red-100" : "bg-green-100"
                  }`}
                >
                  <p className="text-[10px] text-slate-500">
                    LD Performance Estimation
                  </p>
                  <p
                    className={`text-lg font-extrabold ${
                      ldPerformance > 0 ? "text-red-600" : "text-green-600"
                    }`}
                  >
                    ${ldPerformance.toLocaleString()}
                  </p>
                  <p className="text-[9px] text-slate-400">
                    {powerShortfall.toFixed(1)} MW shortfall
                  </p>
                </div>
                <div
                  className={`rounded-xl p-3 text-center ${
                    totalLD > 0 ? "bg-red-600" : "bg-green-600"
                  }`}
                >
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {(() => {
                ...
                const billingCoverage = revenue > 0 ? billing / revenue : 0;
                const billingOk =
                  billingCoverage >= 0.95
                    ? 1
                    : billingCoverage >= 0.85
                    ? 0.5
                    : 0;
                const collectionRatio = billing > 0 ? cashIn / billing : 0;
                const collectionOk =
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <div className="rounded-xl bg-green-50 p-4 text-center">
              ...
              <p className="text-xl font-extrabold text-green-600">
                $
                {(
                  (selectedReport.cashFlow.revenue || evm.bcwp || 0) / 1e6
                ).toFixed(2)}
                M
              </p>
            </div>
            ...
            <div className="rounded-xl bg-slate-50 p-4">
              ...
              {selectedReport.uploads?.cashFlow ? (
                <img
                  ...
                  onClick={() =>
                    openImageViewer(
                      selectedReport.uploads!.cashFlow!.data,
                      "Cash Flow Chart"
                    )
                  }
                ...
                <div className="space-y-2">
                  {[
                    {
                      label: "Revenue",
                      value: selectedReport.cashFlow.revenue || evm.bcwp || 0,
                      color: "bg-green-500",
                      max:
                        Math.max(
                          selectedReport.cashFlow.revenue || 0,
                          selectedReport.cashFlow.cashOut || 0,
                          selectedReport.cashFlow.billing || 0,
                          selectedReport.cashFlow.cashIn || 0
                        ) || 1,
                    },
                    {
                      label: "Cash Out",
                      value: selectedReport.cashFlow.cashOut || 0,
                      color: "bg-red-500",
                      max:
                        Math.max(
                          selectedReport.cashFlow.revenue || 0,
                    ...
                    {
                      label: "Billing",
                      value: selectedReport.cashFlow.billing || 0,
                      color: "bg-blue-500",
                      max:
                        Math.max(
                          selectedReport.cashFlow.revenue || 0,
                    ...
                    {
                      label: "Cash In",
                      value: selectedReport.cashFlow.cashIn || 0,
                      color: "bg-purple-500",
                      max:
                        Math.max(
                          selectedReport.cashFlow.revenue || 0,
                    ...
                    <div key={label} className="flex items-center gap-2">
                      <span className="text-[10px] w-14 text-slate-500">
                        {label}
                      </span>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <strong className="text-red-600">
                    ${((selectedReport.cashFlow.cashOut || 0) / 1e6).toFixed(2)}
                    M
                  </strong>
                </div>
                <div className="flex justify-between text-xs">
                  ...
                  <strong className="text-blue-600">
                    ${((selectedReport.cashFlow.billing || 0) / 1e6).toFixed(2)}
                    M
                  </strong>
                </div>
                <div className="flex justify-between text-xs">
              ...
              <div className="flex justify-around">
                {[
                  ...
                  <div key={qr.key} className="text-center">
                    {selectedReport.uploads?.[
                      qr.key as keyof typeof selectedReport.uploads
                    ] ? (
                      <img
                        src={
                          (
                      ...
                      <div
                        className={`w-12 h-12 bg-slate-200 rounded border-2 ${qr.color} flex items-center justify-center text-lg`}
                      >
                        📷
                      </div>
                    )}
                    <p
                      className={`text-[8px] font-semibold mt-1 ${qr.textColor}`}
                    >
                      {qr.label}
                    </p>
                  </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            {(() => {
              ...
              const ratio =
                (cf.cashOut || 1) > 0
                  ? (cf.cashIn || 0) / (cf.cashOut || 1)
                  : 0;
              const status =
                ratio >= 1 ? "green" : ratio >= 0.8 ? "amber" : "red";
              return (
                <div
                  className={`rounded-lg p-3 ${
                    status === "green"
            ...
            {(() => {
              ...
              const burnRate =
                cf.cashBurnRate ||
                (cf.cashOut || 0) / (selectedReport.weekNo || 1);
              return (
            ...
            {(() => {
              const cf = selectedReport.cashFlow;
              const ratio =
                (evm.bcwp || 1) > 0 ? (cf.cashIn || 0) / (evm.bcwp || 1) : 0;
              const status =
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              const data =
                (
                  selectedReport.epcc as unknown as Record<
                    string,
                    { plan?: number; actual?: number; weight?: number }
                  >
                )[key] || {};
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div ref={sCurveRef} className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          {selectedReport?.uploads?.sCurveGeneral ? (
            <img
              ...
              onClick={() =>
                openImageViewer(
                  (selectedReport.uploads!.sCurveGeneral as { data: string })
                    .data,
                  "S-Curve Progress"
                )
              }
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          <div className="flex justify-around">
            <GaugeChart
              value={evm.spiValue || 0}
              label="SPI"
              color={spiColor}
            />
            <GaugeChart
              value={evm.cpiValue || 0}
              label="CPI"
              color={cpiColor}
            />
          </div>
          <div className="mt-4 grid grid-cols-4 gap-2 text-center text-xs">
            <div className="rounded-lg bg-slate-50 p-2">
              ...
              <p className="font-bold">
                ${((evm.bcws || 0) / 1e6).toFixed(1)}M
              </p>
            </div>
            <div className="rounded-lg bg-slate-50 p-2">
              <p className="text-slate-500">BCWP</p>
              <p className="font-bold">
                ${((evm.bcwp || 0) / 1e6).toFixed(1)}M
              </p>
            </div>
            <div className="rounded-lg bg-slate-50 p-2">
              <p className="text-slate-500">ACWP</p>
              <p className="font-bold">
                ${((evm.acwp || 0) / 1e6).toFixed(1)}M
              </p>
            </div>
            <div className="rounded-lg bg-slate-50 p-2">
      ...
      */}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-lg bg-green-50 p-3 text-center">
              <p className="text-2xl font-extrabold text-green-600">
                {(quality as Record<string, Record<string, number>>)
            ...
            <div className="rounded-lg bg-amber-50 p-3 text-center">
              <p className="text-2xl font-extrabold text-amber-600">
                {(quality as Record<string, Record<string, number>>)
                  ?.certificate?.underApplication || 0}
              </p>
        ...
        (() => {
          const q = selectedReport.quality as unknown as {
            headOffice?: {
              afi?: Record<
                string,
                { fail?: number; ongoing?: number; pass?: number }
              >;
              ncr?: {
                ownerToContractor?: Record<
                  string,
                  { open?: number; closed?: number }
                >;
                contractorToVendor?: Record<
                  string,
                  { open?: number; closed?: number }
                >;
              };
          ...
          disciplines.forEach((d) => {
            ...
            hoAfiTotal +=
              (hoAfi.fail || 0) + (hoAfi.ongoing || 0) + (hoAfi.pass || 0);
            hoAfiPass += hoAfi.pass || 0;

            const soAfi = q?.siteOffice?.afi?.[d] || {};
            soAfiTotal +=
              (soAfi.fail || 0) + (soAfi.ongoing || 0) + (soAfi.pass || 0);
            soAfiPass += soAfi.pass || 0;

            ["ownerToContractor", "contractorToVendor"].forEach((src) => {
              const hoNcr =
                q?.headOffice?.ncr?.[
                  src as "ownerToContractor" | "contractorToVendor"
                ]?.[d] || {};
          ...
          const ndtTotal =
            (welding?.ndtAccepted || 0) + (welding?.ndtRejected || 0);
          const rejRate =
            ndtTotal > 0 ? ((welding?.ndtRejected || 0) / ndtTotal) * 100 : 0;
          const weldPlan = welding?.rejectionRatePlan || 2;
          const weldStatus =
            rejRate <= weldPlan
              ? "Pass"
              : rejRate <= weldPlan * 1.5
              ? "Warning"
              : "Fail";
          ...
          const certTotal =
            (cert?.completed || 0) +
            (cert?.underApplication || 0) +
            (cert?.notYetApplied || 0);
          ...
          const disciplineData = disciplines.map((d) => {
            ...
            const hoNcrO2C = q?.headOffice?.ncr?.ownerToContractor?.[d] || {
              open: 0,
              closed: 0,
            };
            const hoNcrC2V = q?.headOffice?.ncr?.contractorToVendor?.[d] || {
              open: 0,
              closed: 0,
            };
            const soNcrO2C = q?.siteOffice?.ncr?.ownerToContractor?.[d] || {
              open: 0,
              closed: 0,
            };
            const soNcrC2V = q?.siteOffice?.ncr?.contractorToVendor?.[d] || {
              open: 0,
              closed: 0,
            };

            const hoPunchO2C = q?.headOffice?.punchList?.ownerToContractor?.[
              d
            ] || { open: 0, closed: 0 };
            const hoPunchC2V = q?.headOffice?.punchList?.contractorToVendor?.[
              d
            ] || { open: 0, closed: 0 };
            const soPunchO2C = q?.siteOffice?.punchList?.ownerToContractor?.[
              d
            ] || { open: 0, closed: 0 };
            const soPunchC2V = q?.siteOffice?.punchList?.contractorToVendor?.[
              d
            ] || { open: 0, closed: 0 };
            ...
            return {
              ...
              hoAfi: {
                fail: hoAfi.fail || 0,
                ongoing: hoAfi.ongoing || 0,
                pass: hoAfi.pass || 0,
              },
              soAfi: {
                fail: soAfi.fail || 0,
                ongoing: soAfi.ongoing || 0,
                pass: soAfi.pass || 0,
              },
              hoNcr: {
                open: (hoNcrO2C.open || 0) + (hoNcrC2V.open || 0),
                closed: (hoNcrO2C.closed || 0) + (hoNcrC2V.closed || 0),
              },
              soNcr: {
                open: (soNcrO2C.open || 0) + (soNcrC2V.open || 0),
                closed: (soNcrO2C.closed || 0) + (soNcrC2V.closed || 0),
              },
              hoPunch: {
                open: (hoPunchO2C.open || 0) + (hoPunchC2V.open || 0),
                closed: (hoPunchO2C.closed || 0) + (hoPunchC2V.closed || 0),
              },
              soPunch: {
                open: (soPunchO2C.open || 0) + (soPunchC2V.open || 0),
                closed: (soPunchO2C.closed || 0) + (soPunchC2V.closed || 0),
              },
            };
          });
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                ...
                <div className="rounded-xl bg-teal-50 p-4">
                  <p className="text-xs font-semibold text-teal-700 mb-3">
                    🏢 Head Office - AFI Status
                  </p>
                  <div className="space-y-2">
                    {disciplineData.map((d) => {
                      const total =
                        d.hoAfi.fail + d.hoAfi.ongoing + d.hoAfi.pass;
                      return (
                        total > 0 && (
                          <div
                            key={d.discipline}
                          ...
                          >
                            <span className="text-[10px] w-20 capitalize text-slate-600">
                              {d.discipline}
                            </span>
                ...
                <div className="rounded-xl bg-purple-50 p-4">
                  <p className="text-xs font-semibold text-purple-700 mb-3">
                    🏗️ Site Office - AFI Status
                  </p>
                  <div className="space-y-2">
                    {disciplineData.map((d) => {
                      const total =
                        d.soAfi.fail + d.soAfi.ongoing + d.soAfi.pass;
                      return (
                        total > 0 && (
                          <div
                            key={d.discipline}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                ...
                <div className="rounded-xl bg-slate-50 p-4">
                  <p className="text-xs font-semibold text-slate-700 mb-3">
                    🏢 Head Office - NCR & Punch List
                  </p>
                  <table className="w-full text-[10px]">
                    <thead>
                      <tr className="bg-slate-200">
                        <th className="p-1.5 text-left">Discipline</th>
                        <th className="p-1.5 text-center" colSpan={2}>
                          NCR
                        </th>
                        <th className="p-1.5 text-center" colSpan={2}>
                          Punch
                      ...
                      <tr className="bg-slate-100">
                        ...
                        <th className="p-1 text-center text-green-600">
                          Closed
                        </th>
                    ...
                    <tbody>
                      {disciplineData.map((d) => (
                        ...
                        >
                          <td className="p-1.5 capitalize font-medium">
                            {d.discipline}
                          </td>
                          <td
                          ...
                          <td className="p-1.5 text-center text-green-600">
                            {d.hoNcr.closed}
                          </td>
                          <td
                            className={`p-1.5 text-center ${
                      ...
                      <tr className="bg-slate-100 font-bold">
                        ...
                        <td className="p-1.5 text-center text-red-600">
                          {hoNcrOpen}
                        </td>
                        <td className="p-1.5 text-center text-green-600">
                          {hoNcrClosed}
                        </td>
                        <td className="p-1.5 text-center text-amber-600">
                          {hoPunchOpen}
                        </td>
                        <td className="p-1.5 text-center text-green-600">
                          {hoPunchClosed}
                  ...
                  <div className="h-3 bg-slate-200 rounded-full overflow-hidden flex">
                    {(cert?.completed || 0) > 0 && (
                      <div
                        ...
                        style={{
                          width: `${
                            ((cert?.completed || 0) / certTotal) * 100
                          }%`,
                        }}
                      />
                    )}
                    {(cert?.underApplication || 0) > 0 && (
                      <div
                        className="bg-amber-500"
                        style={{
                          width: `${
                            ((cert?.underApplication || 0) / certTotal) * 100
                          }%`,
                        }}
                      />
                    )}
                    {(cert?.notYetApplied || 0) > 0 && (
                      <div
                        className="bg-slate-400"
                        style={{
                          width: `${
                            ((cert?.notYetApplied || 0) / certTotal) * 100
                          }%`,
                        }}
                      />
                    )}
      ...
      {((selectedReport?.milestonesSchedule?.length ?? 0) > 0 ||
        (selectedReport?.milestonesPayment?.length ?? 0) > 0) && (
        <div className="grid gap-5 lg:grid-cols-2">
          {/* Schedule Milestones Table */}
          <div className="rounded-2xl bg-white p-5 shadow-sm">
            <h3 className="mb-4 text-sm font-bold text-violet-600">
              📅 Schedule Milestones
            </h3>
            <div className="max-h-60 overflow-y-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  ...
                  {(!selectedReport?.milestonesSchedule ||
                    selectedReport.milestonesSchedule.length === 0) && (
                    <tr>
                      <td
                        colSpan={5}
          ...
          <div className="rounded-2xl bg-white p-5 shadow-sm">
            <h3 className="mb-4 text-sm font-bold text-amber-600">
              💰 Payment Milestones
            </h3>
            <div className="max-h-60 overflow-y-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  ...
                  {(!selectedReport?.milestonesPayment ||
                    selectedReport.milestonesPayment.length === 0) && (
                    <tr>
                      <td
                        colSpan={5}
                        className="p-4 text-center text-slate-400"
      ...
      {(selectedReport?.thisWeekActivities || selectedReport?.nextWeekPlan) && (
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="rounded-2xl bg-white p-5 shadow-sm overflow-hidden">
            <h3 className="mb-4 text-sm font-bold text-teal-600">
              📋 This Week Activities
            </h3>
            <div className="max-h-64 overflow-y-auto overflow-x-hidden space-y-3">
              {Object.entries(
                (selectedReport?.thisWeekActivities || {}) as unknown as Record<
                  string,
                  string[]
                >
              ).map(([cat, items]) => {
                ...
                return (
                  <div key={cat}>
                    <p className="text-xs font-semibold text-teal-600 capitalize mb-1">
                      {cat}
                    </p>
                    <ul className="pl-4 space-y-0.5">
                      {filteredItems.map((item, j) => (
                        <li
                          key={j}
                          className="text-xs text-slate-600 list-disc break-all whitespace-normal"
                        >
                          {item}
                        </li>
                      ))}
              ...
              {Object.keys(selectedReport?.thisWeekActivities || {}).length ===
                0 && (
          ...
          <div className="rounded-2xl bg-white p-5 shadow-sm overflow-hidden">
            ...
            <div className="max-h-64 overflow-y-auto overflow-x-hidden space-y-3">
              {Object.entries(
                (selectedReport?.nextWeekPlan || {}) as unknown as Record<
                  string,
                  string[]
                >
              ).map(([cat, items]) => {
