...
export default function ReportsPage() {
  ...
  const openEdit = async (reportSummary: ReportSummary) => {
    setLoadingReportId(reportSummary.id);
    const fullReport = await fetchReportById(reportSummary.id);
  ...
  const openView = async (reportSummary: ReportSummary) => {
    setLoadingReportId(reportSummary.id);
    const fullReport = await fetchReportById(reportSummary.id);
    setLoadingReportId(null);
    if (fullReport) {
  ...
  return (
    <div>
      ...
      {projects.length === 0 ? (
        <div className="rounded-2xl bg-amber-50 p-8 text-center">
          ...
          <p className="text-sm text-amber-600">
            You need to create a project first before creating reports.
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-slate-50 p-12 text-center">
          ...
          <p className="text-sm text-slate-500">
            {filterProjectId === "all"
              ...
              : "Try selecting a different project or create a new report"}
          </p>
          <button
            onClick={openCreate}
        ...
        filteredReports.map((r) => {
          const isLoadingThis = loadingReportId === r.id;
          return (
            <div
              key={r.id}
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs">
                  {r.cashFlowStatus === "green"
                    ? "🟢"
                    : r.cashFlowStatus === "yellow"
                    ? "🟡"
                    : "🔴"}
                </span>
                <span
                ...
                <button
                  ...
                  className="rounded-lg bg-teal-50 p-2 text-teal-600 hover:bg-teal-100 disabled:opacity-50"
                  title="View Report"
                >
                  {isLoadingThis ? (
      ...
      <ViewReportModal
        ...
        projectName={
          viewingReport ? getProjectName(viewingReport.projectId) : ""
        }
      />
    </div>
  );
