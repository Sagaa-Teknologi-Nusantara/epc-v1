...
export function ViewReportModal({
  ...
  const overallProgress = report.overallProgress || {
    plan: 0,
    actual: 0,
    variance: 0,
  };
  const hse = report.hse || {
    lagging: {},
    leading: {},
    manpower: { total: 0 },
    safeHours: 0,
  ...
  const cfStatus = calculateCashFlowStatus(
    cashFlow as unknown as Record<string, number>,
    evm.bcwp || 0
  );
  ...
  const spiColor =
    (evmCalc.spiValue || 0) >= 1
      ? "text-green-600"
      : (evmCalc.spiValue || 0) >= 0.9
      ? "text-amber-600"
      : "text-red-600";
  const cpiColor =
    (evmCalc.cpiValue || 0) >= 1
      ? "text-green-600"
      : (evmCalc.cpiValue || 0) >= 0.9
      ? "text-amber-600"
      : "text-red-600";
  ...
  return (
    ...
    >
      <div
        className="w-[95%] max-w-6xl max-h-[90vh] overflow-hidden rounded-2xl bg-white shadow-xl"
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-200 bg-gradient-to-r from-teal-500 to-teal-600 p-4 text-white">
          <div>
            <h2 className="text-lg font-bold">
              {report.docNo || `Week ${report.weekNo} Report`}
            </h2>
            <p className="text-sm opacity-90">
              {projectName} | {report.periodStart} - {report.periodEnd}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <span
              className={`rounded-full px-3 py-1 text-xs font-semibold ${
                report.status === "Issued" ? "bg-green-500" : "bg-amber-500"
              }`}
            >
              {report.status}
            </span>
        ...
        <div className="p-5 max-h-[65vh] overflow-y-auto bg-slate-100">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="rounded-xl bg-white p-4 shadow-sm">
                  ...
                  >
                    {(overallProgress.variance || 0) >= 0 ? "+" : ""}
                    {(overallProgress.variance || 0).toFixed(1)}% vs plan
                  </p>
                </div>
                <div
                ...
                <div className="rounded-xl bg-white p-4 shadow-sm">
                  ...
                  <p className="text-xs text-slate-500">
                    TRIR: {(hse.trir || 0).toFixed(2)}
                  </p>
                </div>
                <div
                  className={`rounded-xl p-4 shadow-sm ${
                    cfStatus.overallStatus === "green"
                      ? "bg-green-50"
                      : cfStatus.overallStatus === "yellow"
                      ? "bg-amber-50"
                      : "bg-red-50"
                  }`}
                >
                  ...
                  <p className="text-xs text-slate-500">
                    Score: {(cfStatus.overallScore * 100).toFixed(0)}%
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl bg-white p-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                  <div className="rounded-lg bg-green-50 p-3 text-center">
                    ...
                    <p className={`text-xl font-extrabold ${spiColor}`}>
                      {(evm.spiValue || 0).toFixed(3)}
                    </p>
                  </div>
                  <div className="rounded-lg bg-amber-50 p-3 text-center">
                    <p className="text-xs text-slate-500">CPI</p>
                    <p className={`text-xl font-extrabold ${cpiColor}`}>
                      {(evm.cpiValue || 0).toFixed(3)}
                    </p>
                  </div>
                  <div className="rounded-lg bg-slate-50 p-3 text-center">
                    <p className="text-xs text-slate-500">BCWS</p>
                    <p className="text-lg font-bold">
                      ${((evm.bcws || 0) / 1e6).toFixed(2)}M
                    </p>
                  </div>
                  <div className="rounded-lg bg-slate-50 p-3 text-center">
                  ...
                  <div className="rounded-lg bg-slate-50 p-3 text-center">
                    ...
                    <p className="text-lg font-bold">
                      ${((evm.acwp || 0) / 1e6).toFixed(2)}M
                    </p>
                  </div>
                  <div className="rounded-lg bg-slate-50 p-3 text-center">
                    <p className="text-xs text-slate-500">BAC</p>
                    <p className="text-lg font-bold">
                      ${((evm.bac || 0) / 1e6).toFixed(2)}M
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl bg-white p-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  >
                    ...
                    <p className={`text-3xl font-extrabold ${spiColor}`}>
                      {(evmCalc.spiValue || 0).toFixed(3)}
                    </p>
                  ...
                  >
                    ...
                    <p className={`text-3xl font-extrabold ${cpiColor}`}>
                      {(evmCalc.cpiValue || 0).toFixed(3)}
                    </p>
                  ...
                  <div className="rounded-lg bg-blue-50 p-4 text-center">
                    ...
                    <p className="text-2xl font-extrabold text-blue-600">
                      ${((evmCalc.bac || 0) / 1e6).toFixed(2)}M
                    </p>
                    <p className="text-xs text-slate-500">BAC</p>
                  </div>
                  <div
                    className={`rounded-lg p-4 text-center ${
                      (evmCalc.vac || 0) >= 0 ? "bg-green-50" : "bg-red-50"
                    }`}
                  >
                    <p className="text-xs text-slate-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="rounded-lg bg-purple-50 p-4 text-center">
                    ...
                    <p className="text-2xl font-extrabold text-purple-600">
                      ${((evmCalc.eacTypical || 0) / 1e6).toFixed(2)}M
                    </p>
                  ...
                  <div className="rounded-lg bg-slate-50 p-4 text-center">
                    ...
                    <p className="text-2xl font-extrabold text-slate-600">
                      ${((evmCalc.eacAtypical || 0) / 1e6).toFixed(2)}M
                    </p>
                  ...
                  <div className="rounded-lg bg-teal-50 p-4 text-center">
                    ...
                    <p className="text-2xl font-extrabold text-teal-600">
                      ${((evmCalc.eacCombined || 0) / 1e6).toFixed(2)}M
                    </p>
                    <p className="text-xs text-slate-400">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div
                    className={`rounded-lg p-3 text-center ${
                      cfBillingCoverage >= 0.95
                        ? "bg-green-50"
                        : cfBillingCoverage >= 0.85
                        ? "bg-amber-50"
                        : "bg-red-50"
                    }`}
                  >
                    ...
                    >
                      {(cfBillingCoverage * 100).toFixed(1)}%
                    </p>
                    <p className="text-xs text-slate-400">
                      = Billing / Revenue
                    </p>
                  ...
                  <div
                    className={`rounded-lg p-3 text-center ${
                      cfCashCollection >= 0.9
                        ? "bg-green-50"
                        : cfCashCollection >= 0.8
                        ? "bg-amber-50"
                        : "bg-red-50"
                    }`}
                  >
                    ...
                    >
                      {(cfCashCollection * 100).toFixed(1)}%
                    </p>
                    <p className="text-xs text-slate-400">
                      = Cash In / Billing
                    </p>
                  ...
                  <div
                    className={`rounded-lg p-3 text-center ${
                      cfCashBalance >= 0 ? "bg-green-50" : "bg-red-50"
                    }`}
                  >
                    <p className="text-xs text-slate-500">Cash Balance</p>
                    <p
                      className={`text-lg font-bold ${
                        cfCashBalance >= 0 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      ${(cfCashBalance / 1e6).toFixed(2)}M
                    </p>
                    <p className="text-xs text-slate-400">
                      = Cash In - Cash Out
                    </p>
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Based on all ratio calculations
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      >
                        ...
                        <div
                          className={`inline-block ${color} text-white px-4 py-2 rounded-lg`}
                        >
                          <p className="text-xl font-extrabold">
                            {(
                              hse.lagging as unknown as Record<string, number>
                            )?.[k] || 0}
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      >
                        <p className="text-xs text-slate-500 mb-1">{l}</p>
                        <p className="text-xl font-extrabold text-green-600">
                          {(hse.leading as unknown as Record<string, number>)?.[
                            k
                          ] || 0}
                        </p>
                      </div>
          ...
          {activeTab === "quality" &&
            (() => {
              ...
              const rate =
                total > 0 ? ((welding?.ndtRejected || 0) / total) * 100 : 0;
              ...
              disciplines.forEach((d) => {
                ...
                ["ownerToContractor", "contractorToVendor"].forEach((src) => {
                  hoNcrOpen +=
                    q?.headOffice?.ncr?.[src as "ownerToContractor"]?.[d]
                      ?.open || 0;
                  hoPunchOpen +=
                    q?.headOffice?.punchList?.[src as "ownerToContractor"]?.[d]
                      ?.open || 0;
                  soNcrOpen +=
                    q?.siteOffice?.ncr?.[src as "ownerToContractor"]?.[d]
                      ?.open || 0;
                  soPunchOpen +=
                    q?.siteOffice?.punchList?.[src as "ownerToContractor"]?.[d]
                      ?.open || 0;
                });
              });
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        ...
                        <tr className="bg-slate-50">
                          ...
                          <th className="p-1 text-center text-amber-600">
                            Ongoing
                          </th>
                    ...
                    <div className="rounded-xl bg-white p-4 shadow-sm">
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {disciplines.map((d) => {
                            const hoOpen =
                              (q?.headOffice?.ncr?.ownerToContractor?.[d]
                                ?.open || 0) +
                              (q?.headOffice?.ncr?.contractorToVendor?.[d]
                                ?.open || 0);
                            const soOpen =
                              (q?.siteOffice?.ncr?.ownerToContractor?.[d]
                                ?.open || 0) +
                              (q?.siteOffice?.ncr?.contractorToVendor?.[d]
                                ?.open || 0);
                            return (
                              <tr key={d} className="border-b">
                    ...
                    <div className="rounded-xl bg-white p-4 shadow-sm">
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {disciplines.map((d) => {
                            const hoOpen =
                              (q?.headOffice?.punchList?.ownerToContractor?.[d]
                                ?.open || 0) +
                              (q?.headOffice?.punchList?.contractorToVendor?.[d]
                                ?.open || 0);
                            const soOpen =
                              (q?.siteOffice?.punchList?.ownerToContractor?.[d]
                                ?.open || 0) +
                              (q?.siteOffice?.punchList?.contractorToVendor?.[d]
                                ?.open || 0);
                            return (
                              <tr key={d} className="border-b">
                                <td className="p-1.5 capitalize">{d}</td>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        ...
                        <div className="rounded-lg bg-white p-2 text-center">
                          ...
                          <p
                            className={`text-lg font-bold ${
                              rate <= plan ? "text-green-600" : "text-red-600"
                            }`}
                          >
                            {rate.toFixed(1)}%
                          </p>
          ...
          {activeTab === "milestones" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Schedule Milestones */}
              <div className="rounded-xl bg-white p-4 shadow-sm">
                <h3 className="text-sm font-bold text-violet-600 mb-4">
                  📅 Schedule Milestones
                </h3>
              ...
              {/* Payment Milestones */}
              <div className="rounded-xl bg-white p-4 shadow-sm">
                <h3 className="text-sm font-bold text-amber-600 mb-4">
                  💳 Payment Milestones
                </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="rounded-xl bg-white p-4 shadow-sm">
                ...
                {(
                  ...
                  const items =
                    (
                      report.thisWeekActivities as unknown as Record<
                        string,
                        string[]
                      >
                    )?.[key] || [];
                  ...
                  return (
                    <div key={key} className="mb-3">
                      <p className="text-xs font-semibold text-slate-600 capitalize mb-1">
                        {key}
                      </p>
              ...
              <div className="rounded-xl bg-white p-4 shadow-sm">
                ...
                {(
                  ...
                  const items =
                    (
                      report.nextWeekPlan as unknown as Record<string, string[]>
                    )?.[key] || [];
                  if (items.length === 0 || (items.length === 1 && !items[0]))
                    return null;
                  return (
          ...
          {activeTab === "uploads" && (
            <div className="space-y-4">
              ...
              <div className="rounded-xl bg-white p-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    {
                      key: "sCurveEngineering",
                      label: "Engineering",
                      color: "from-blue-400 to-blue-600",
                    },
                    {
                      key: "sCurveProcurement",
                      label: "Procurement",
                      color: "from-amber-400 to-amber-600",
                    },
                    {
                      key: "sCurveConstruction",
                      label: "Construction",
                      color: "from-purple-400 to-purple-600",
                    },
                  ].map((item) => {
                    const upload = (
                      report.uploads as Record<
                        string,
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`bg-gradient-to-r ${item.color} text-white text-xs font-semibold p-2 text-center`}
                        >
                          {item.label}
                        </div>
                        {upload?.data ? (
                          <img
                            src={upload.data}
                            alt={item.label}
                            className="w-full h-24 object-cover hover:scale-105 transition cursor-pointer"
                          />
                        ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* QR Codes */}
                <div className="rounded-xl bg-white p-4 shadow-sm">
                  <h3 className="text-sm font-bold text-purple-600 mb-4">
                    📱 QR Codes
                  </h3>
                    ...
                    {[
                      ...
                      return (
                        <div key={qr.key} className="text-center">
                          <div
                            className={`${qr.color} text-white text-[10px] font-semibold py-1 px-2 rounded-t`}
                          >
                            {qr.label}
                          </div>
                          {upload?.data ? (
                            <img
                              src={upload.data}
                              alt={qr.label}
                              className="w-full h-16 object-cover rounded-b border border-t-0 border-slate-200"
                            />
                          ) : (
        ...
        <div className="flex justify-end border-t border-slate-200 p-4 bg-white">
          <button
            onClick={onClose}
            className="px-6 py-2 rounded-lg bg-slate-100 text-slate-600 font-medium text-sm hover:bg-slate-200"
          >
            Close
          </button>
        </div>
