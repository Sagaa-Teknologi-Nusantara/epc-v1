...
export class PDFExporter {
  ...
  addHeader(): void {
    ...
    if (this.options.periodStart && this.options.periodEnd) {
      this.pdf.text(
        `Period: ${this.options.periodStart} - ${this.options.periodEnd}`,
        this.pageWidth / 2,
        infoY,
        { align: "center" }
      );
  ...
  addStatsRow(
    ...
    const boxWidth =
      (this.contentWidth - (items.length - 1) * 3) / items.length;
    ...
    items.forEach((item, index) => {
      ...
      this.pdf.text(item.value, x + boxWidth / 2, this.currentY + 7, {
        align: "center",
      });
      ...
      this.pdf.text(item.label, x + boxWidth / 2, this.currentY + 13, {
        align: "center",
      });
    });
  ...
  addText(
    ...
    options?: { align?: "left" | "center" | "right"; italic?: boolean }
  ): void {
    this.checkNewPage(6);

    this.pdf.setFontSize(size === "small" ? 8 : 9);
    this.pdf.setTextColor(color.r, color.g, color.b);
    this.pdf.setFont("helvetica", options?.italic ? "italic" : "normal");
    ...
    this.pdf.text(text, x, this.currentY, {
      align: options?.align === "left" ? undefined : options?.align,
    });

    // Reset font
    if (options?.italic) this.pdf.setFont("helvetica", "normal");
  ...
  addTable(
    ...
    const rowHeight = options?.rowHeight || 8; // Increased for better breathing room
    const headerHeight = 9;

    // Check if we have space for at least header + 1 row
    this.checkNewPage(headerHeight + rowHeight + 4);

    const { teal, white, darkGray, lightGray } = COLORS;
    let x = this.margin;
    ...
    headers.forEach((header) => {
      ...
      let headerX = x + 2; // Default left padding

      if (align === "center") headerX = x + header.width / 2;
      else if (align === "right") headerX = x + header.width - 2;

      this.pdf.text(header.label, headerX, this.currentY + 6, {
    ...
    rows.forEach((row, rowIndex) => {
      // Check for new page before each row
      this.checkNewPage(rowHeight + 2);

      // Alternating row background (subtle)
      if (rowIndex % 2 === 0) {
        this.pdf.setFillColor(248, 250, 252); // Very light gray #f8fafc
      } else {
        this.pdf.setFillColor(white.r, white.g, white.b);
      }
      this.pdf.rect(
        this.margin,
        this.currentY,
      ...
      // Bottom border only (cleaner look)
      ...
      row.forEach((cell, cellIndex) => {
        const align = cell.align || headers[cellIndex]?.align || "left";
        const cellWidth = headers[cellIndex]?.width || 20;

        if (cell.type === "badge") {
          // Badge Rendering
          const badgeBg = cell.backgroundColor || teal;
          const badgeText = cell.text;
          const badgeWidth = this.pdf.getTextWidth(badgeText) + 6;
          const badgeHeight = rowHeight - 3;

          // Calculate badge position specifically for centering
          let badgeX = x + 2;
          if (align === "center") badgeX = x + (cellWidth - badgeWidth) / 2;
          else if (align === "right") badgeX = x + cellWidth - badgeWidth - 2;

          // Draw badge pill
          this.pdf.setFillColor(badgeBg.r, badgeBg.g, badgeBg.b);
          ...
          // Draw badge text
          this.pdf.setTextColor(white.r, white.g, white.b);
          this.pdf.setFontSize(fontSize - 1); // Slightly smaller for badges
          this.pdf.setFont("helvetica", "bold");
          this.pdf.text(
            badgeText,
            badgeX + badgeWidth / 2,
            this.currentY + rowHeight / 2 + 1.5,
            { align: "center" }
          );
          ...
          // Text Rendering
          const color = cell.color || darkGray;
          this.pdf.setTextColor(color.r, color.g, color.b);
          ...
          if (
            this.pdf.getTextWidth(displayText) > maxWidth &&
            displayText.length > 3
          ) {
            // Simple truncation logic (could be improved)
            while (
              this.pdf.getTextWidth(displayText + "...") > maxWidth &&
          ...
          else if (align === "right") textX = x + cellWidth - 2;
    ...
    this.currentY += 4; // Extra spacing after table
  }
